...
class HardLabelAttack(PopulationBasedSearch):
    ...
    def _search_space_reduction(self, initial_result, initial_adv_text):
        ...
        for score, replacement in replacements:
            ...
            if results[0].goal_status == GoalFunctionResultStatus.SUCCEEDED and not search_over:
    ...
    def perform_search(self, initial_result):
        ...
        for _ in range(100):
            ...
            for pop_member in population:
                ...
                similarity_scores.append(pop_member.attributes["similarity_score"].cpu().item())

                if num_different_words < num_min_different_words:
                    best_attack = pop_member
                    num_min_different_words = num_different_words
            result, search_over = self.get_goal_results([best_attack.attacked_text])
            if search_over:
                break
            else:
            ...
            result, search_over = self.get_goal_results(child_texts)
            if search_over:
                break
            for i in range(len(results)):
